...
const defaultPermissions: UserPermissions = USER_ROLES.viewer;
...
export function PermissionsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        if (user.permissions) {
          // Merge stored permissions with default to ensure all fields exist
          setPermissions({
            ...defaultPermissions,
            ...user.permissions,
          });
        } else if (user.role) {
          // If only role is stored, use predefined role permissions
          const roleKey = user.role as keyof typeof USER_ROLES;
          if (USER_ROLES[roleKey]) {
            setPermissions(USER_ROLES[roleKey]);
          }
        }
      } catch (e) {
  ...
  const handleSetPermissions = (newPermissions: UserPermissions) => {
